...
def require_auth(f):
    ...
    def decorated(*args, **kwargs):
        ...
        try:
            ...
            # Reload user_credits before validation
            load_credits()
...
def upload_model():
    """
    Endpoint for uploading a new model.
    Accepts form-data (multipart) with:
      - model_file: (a .gguf file)
      - model_name: string
      - visibility: 'public' or 'whitelisted'
      - allowed_addresses: list of addresses (optional)
    """
    MAX_FILE_SIZE_MB = 5000  # Maximum file size in MB
    ALLOWED_EXTENSION = ".gguf"
    ...
    # Identify address (if not admin)
    if admin_key and admin_key == f"Bearer {API_KEY}":
        # Admin can upload in their name or "system name"
        address = "ADMIN"
    else:
    ...
    logger.info(f"Request model_name: {model_name}")
    logger.info(f"Request visibility: {visibility}")
    logger.info(f"Request allowed_addresses_str: {allowed_addresses_str}")
    ...
    extension = os.path.splitext(filename)[1].lower()  # Ensure case-insensitivity
    if extension != ALLOWED_EXTENSION:
        return jsonify({"error": f"Only {ALLOWED_EXTENSION} files are allowed"}), 400

    # Validate file size
    file.seek(0, os.SEEK_END)  # Move the cursor to the end of the file
    file_size_mb = file.tell() / (1024 * 1024)  # Size in MB
    if file_size_mb > MAX_FILE_SIZE_MB:
        return jsonify({"error": f"File size exceeds the limit of {MAX_FILE_SIZE_MB} MB"}), 400
    file.seek(0)  # Reset the cursor to the beginning for saving

    # Handle allowed_addresses
    allowed_addresses = []
    ...
    save_path = os.path.join(MODELS_FOLDER, f"{model_name}{ALLOWED_EXTENSION}")
...
running_models = {}  # Dictionary to track running models

@app.route('/models/<model_name>/start', methods=['POST'])
@require_auth
def start_model(model_name):
    """
    Starts a model using Ollama.
    Creates a Modelfile and executes the `ollama create` and `ollama run` commands.
    """
    global running_models

    # Verifica se o modelo existe nos metadados
    if model_name not in models_metadata:
        return jsonify({"error": f"Model '{model_name}' not found"}), 404

    model_metadata = models_metadata[model_name]
    model_path = model_metadata["file_path"]

    if model_name in running_models:
        return jsonify({"error": f"Model '{model_name}' is already running"}), 400

    try:
        # Cria o arquivo Modelfile
        modelfile_path = os.path.join(MODELS_FOLDER, f"{model_name}.Modelfile")
        with open(modelfile_path, 'w') as modelfile:
            modelfile.write(f"FROM {model_path}\n")

        # Executa o comando `ollama create`
        subprocess.run(["ollama", "create", model_name, "-f", modelfile_path], check=True)

        # Inicia o modelo
        process = subprocess.Popen(["ollama", "run", model_name], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        running_models[model_name] = process

        return jsonify({"message": f"Model '{model_name}' started successfully"}), 200

    except subprocess.CalledProcessError as e:
        logger.error(f"Error starting model '{model_name}': {e}")
        return jsonify({"error": f"Failed to start model '{model_name}'"}), 500
...
def stop_model(model_name):
    ...
    if model_name not in running_models:
        return jsonify({"error": f"Model '{model_name}' is not running"}), 404

    process = running_models.pop(model_name, None)
    if process:
        process.terminate()
        try:
    ...
    return jsonify({"message": f"Model '{model_name}' stopped successfully"}), 200
